...
| LLM              | Claude — Haiku for batch summaries, Sonnet for RAG chat                | Two models split by job: cheap/batch for ingest-time summaries, stronger for user-facing chat. Wrapped behind a thin provider interface so the choice isn't load-bearing.                | Single model for both (wasteful on summaries); OpenAI (fine; abstraction keeps it swappable).                                                                                         |
